    ...
    def create_dev_environment(self):
        ...
        try:
            ...
            try:
                ...
                click.echo(
                    click.style(
                        f"Created new session: {self.dev_session_name}", fg="green"
                    )
                )
                ...
                click.echo(
                    click.style(
                        f"Attached to existing session: {self.dev_session_name}",
                        fg="yellow",
                    )
                )
            ...
            )  # Fixed: using active_window instead of attached_window
            main_window.rename_window("main")

            # Clear and setup main pane
            main_pane = (
                main_window.active_pane
            )  # Fixed: using active_pane instead of attached_pane
            main_pane.send_keys("clear")
            main_pane.send_keys('echo "🚀 Development Area"')

            # Create editor pane
            editor_pane = main_window.split(
                percent=70
            )  # Fixed: using percent instead of percentage
            editor_pane.send_keys("clear")
            editor_pane.send_keys('echo "📝 Editor Ready"')

            # Create bottom pane for git
            git_pane = editor_pane.split(
                percent=30
            )  # Fixed: using percent instead of percentage
        ...
        try:
            try:
                session = self.server.new_session(
                    session_name=self.monitor_session_name
                )
                click.echo(
                    click.style(
                        f"Created new monitoring session: {self.monitor_session_name}",
                        fg="green",
                    )
                )
            except libtmux.exc.TmuxSessionExists:
                ...
                click.echo(
                    click.style(
                        f"Attached to existing monitoring session: {self.monitor_session_name}",
                        fg="yellow",
                    )
                )
            ...
            click.echo(
                click.style(f"❌ Error creating monitoring layout: {e}", fg="red")
            )
