...
# SECURITY: -k removed — crt.sh and web.archive.org have valid public certs;
# bypassing verification just opens these aliases to in-path MITM downgrade.
alias recon-crtsh='f(){ curl -sS "https://crt.sh/?output=json&q=$1" | jq . ; unset -f f; }; f'
alias recon-wayback='f(){ curl -sS "https://web.archive.org/cdx/search/cdx?fl=original&collapse=urlkey&url=*.$1" ; unset -f f; }; f'
